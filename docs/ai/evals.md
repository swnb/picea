...
8. **M23 broadphase/query performance hardening 应该先读哪些路由，而不是直接扫全仓库？**
   - Expected sources: `docs/ai/index.md`, `docs/ai/repo-map.md`, `docs/plans/2026-04-25-picea-physics-engine-production-milestones.md`
   - Pass criteria: 指向 M23 row、`pipeline/broadphase.rs`、`query.rs`、`debug.rs`、`physics_scenarios.rs` 和对应 `rtk proxy` gates；不把 M24+ lab-web 可视化当作当前实现范围。

9. **M24 lab-web visualization foundation 的边界是什么？**
   - Expected sources: `docs/ai/index.md`, `docs/ai/repo-map.md`, `docs/design/physics-engine-upgrade-technical-plan.md`
   - Pass criteria: 能区分 Rust artifact/debug facts、`picea-lab` artifact/schema、`crates/picea-lab/web/src/*` 展示层；不把前端可视化当成 core solver 行为变更。

## Debug

10. **如果某个碰撞 bug 只在某个 step 序列下复现，第一步应该记录什么？**
   - Expected sources: `docs/ai/debug-playbook.md`
   - Pass criteria: 必须提到固定 `StepConfig::dt`、输入序列、最小 repro、行为锁、当前 git/HEAD/status。

11. **为什么 debug 时不能只看“最终看起来没问题”的截图？**
...
12. **如果一个 material response bug 会吃掉切向速度，应该如何锁行为？**
...
- 答案必须引用 expected sources 里的具体文件路径，并说明为什么先读这些来源。
- 答案必须遵守 `AGENTS.md` 的权威顺序：live repo facts 和验证输出优先，当前生产化 milestone 其次，AI routing docs 负责定位。
- 回答实现类任务时必须给出最小行为锁或 targeted test、主要 edit hotspots、以及对应 `rtk proxy` 验证门。
- 回答 routing 类任务时不能先广扫全仓库，也不能把归档 `Scene` / `Context` / `picea-web` / wasm narrative 当成当前默认目标。
- 遇到 freshness / conflict 题，必须明确哪些事实需要本轮重新验证，哪些只是文档或旧记忆。
