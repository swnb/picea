...
| `pipeline` | `SimulationPipeline`、`StepConfig`、`StepReport`、step orchestration、broadphase/narrowphase contact generation。 | 不拥有 world 持久状态。 | `pipeline.rs`, `pipeline/step.rs`, `pipeline/integrate.rs`, `pipeline/contacts.rs`, `pipeline/broadphase.rs`, `pipeline/narrowphase.rs`, `pipeline/joints.rs`, `pipeline/sleep.rs` | `rtk proxy cargo test -p picea --test v1_api_smoke`, `rtk proxy cargo test -p picea --test physics_realism_acceptance`, `rtk proxy cargo test -p picea --lib --tests` |
