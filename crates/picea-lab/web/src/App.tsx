        ...
        <TreeGroup icon={<Box className="h-4 w-4" />} label={t(locale, "tree.bodies")} count={frame.snapshot.bodies.length} locale={locale}>
          {frame.snapshot.bodies.map((body) => {
            ...
            const selectedOwnedCollider =
              selected?.kind === "collider" && bodyColliders.some((collider) => collider.handle === selected.id);
            return (
              <div key={body.handle} className="mb-1">
                <TreeRow
                  ...
                  meta={<BodyTypePill locale={locale} bodyType={body.body_type} />}
                  onClick={() => onSelect({ kind: "body", id: body.handle })}
                />
                <div className="ml-4 border-l border-lab-line/80 pl-2">
                  {bodyColliders.length > 0 ? (
                    ...
                    <div className="px-2 py-1 text-xs text-lab-muted">{t(locale, "tree.empty")}</div>
                  )}
                </div>
              </div>
            );
  ...
  return (
    <button
      ...
      className={cn(
        ...
        level === 1 && "h-7 text-[13px]",
        active && "bg-lab-accent/[0.18] text-lab-text ring-1 ring-lab-accent/40",
        !active && ancestorActive && "bg-lab-accent/[0.08] text-lab-text",
        !active && !ancestorActive && "text-lab-muted hover:bg-white/[0.06] hover:text-lab-text",
      )}
    >
      <span className={cn("shrink-0", level === 1 && "text-lab-muted")}>{icon}</span>
...
function BodyTypePill({ locale, bodyType }: { locale: Locale; bodyType: DebugBody["body_type"] }) {
  return (
    <span
      className={cn(
        "rounded border px-1.5 py-0.5 text-[10px] font-semibold leading-none",
        bodyType === "dynamic" && "border-lab-accent/50 bg-lab-accent/15 text-lab-accent",
        bodyType === "static" && "border-lab-line bg-white/[0.04] text-lab-muted",
        bodyType === "kinematic" && "border-amber-400/45 bg-amber-400/10 text-amber-200",
      )}
    >
